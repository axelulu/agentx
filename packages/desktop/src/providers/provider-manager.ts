...
import type { DesktopProviderConfig } from "../types.js";
...
export class ProviderManager {
  private providers = new Map<string, DesktopProviderConfig>();
  private activeId: string | null = null;

  setProvider(config: DesktopProviderConfig): void {
  ...
  getActiveProvider(): DesktopProviderConfig | null {
  ...
  private createStreamFnForProvider(config: DesktopProviderConfig): StreamFn {
    switch (config.type) {
      case "openai":
      case "custom":
...
function getDefaultModelForType(type: DesktopProviderConfig["type"]): string {
  switch (type) {
    case "openai":
      return "gpt-4o";
    case "anthropic":
