/**
 * Tauri IPC Bridge — implements the same ElectronAPI interface
 * using @tauri-apps/api instead of Electron's ipcRenderer.
 *
 * This file is loaded only in Tauri environments (detected via __TAURI_INTERNALS__).
 */
...
// ---------------------------------------------------------------------------
// Helper to create event listeners compatible with Electron's pattern
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// The Tauri bridge — same shape as ElectronAPI
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Convert local-file:// URLs to Tauri asset URLs
// ---------------------------------------------------------------------------
export function convertLocalFileUrl(url: string): string {
...
export function initTauriBridge(): void {
  // Set window.api to the Tauri bridge
  (window as unknown as { api: ElectronAPI }).api = tauriBridge;

  // Set up drag-and-drop
  setupDragDrop().catch((err) => {
    console.error("[TauriBridge] Failed to setup drag-drop:", err);
  });
